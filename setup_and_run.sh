...
(cd src/static/js && pm2 start server.js --name "whatsapp-bot")

# --- 4. FLASK SERVER (PYTHON) ---
echo "--> Starting Python Flask Server with PM2..."
pm2 stop python-bot 2>/dev/null || true
pm2 delete python-bot 2>/dev/null || true
pm2 start "$ENV_NAME/bin/python" --name "python-bot" -- -m src.main
...
echo "Python logs:   pm2 logs python-bot"
echo "WhatsApp logs: pm2 logs whatsapp-bot"
echo ""
